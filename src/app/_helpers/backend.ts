...
export class BackofficeApiService implements IBackofficeApiService {
    ...
    totals_GetTopTen(): Observable<TopTen[]> {
        let url_ = this.baseUrl + "/Totals/TopTen";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processTotals_GetTopTen(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processTotals_GetTopTen(<any>response_);
                } catch (e) {
                    return <Observable<TopTen[]>><any>_observableThrow(e);
                }
            } else
                return <Observable<TopTen[]>><any>_observableThrow(response_);
        }));
    }

    protected processTotals_GetTopTen(response: HttpResponseBase): Observable<TopTen[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 401) {
            ...
            if (Array.isArray(resultData200)) {
                result200 = [] as any;
                for (let item of resultData200)
                    result200!.push(TopTen.fromJS(item));
            }
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
        ...
        return _observableOf<TopTen[]>(<any>null);
    }
...
export class TopTen implements ITopTen {
    ...
    static fromJS(data: any): TopTen {
        data = typeof data === 'object' ? data : {};
        let result = new TopTen();
        result.init(data);
        return result;
    }
